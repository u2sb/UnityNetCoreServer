  ...
  public class FileCache : IDisposable
  {
    ...
    public (bool, byte[]) Find(string key)
    {
      using (new ReadLock(_lockEx))
      {
        ...
        return !_entriesByKey.TryGetValue(key, out var cacheValue)
    ...
    public bool InsertPath(string path, string prefix = "/", string filter = "*.*", TimeSpan timeout = new(),
    ...
    private readonly Dictionary<string, MemCacheEntry> _entriesByKey = new();
    private readonly Dictionary<string, HashSet<string>> _entriesByPath = new();
    private readonly Dictionary<string, FileCacheEntry> _pathsByKey = new();
