...
namespace NetCoreServer
{
  ...
  public class UdsServer : IDisposable
  {
    ...
    protected virtual UdsSession CreateSession()
    {
      return new UdsSession(this);
    }
    ...
    // Server acceptor
    private Socket _acceptorSocket = null!;
    private SocketAsyncEventArgs _acceptorEventArg = new();
    ...
    public virtual bool Multicast<T>(T data) where T : struct
    {
      var span = MemoryMarshal.CreateReadOnlySpan(ref data, 1);
      return Multicast(span);
    }
    ...
    /// <param name="data">Data to send</param>
    /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
    public virtual bool Multicast<T>(ReadOnlySpan<T> data) where T : struct
    {
    ...
    internal void OnConnectingInternal(UdsSession session)
    {
      OnConnecting(session);
    }

    internal void OnConnectedInternal(UdsSession session)
    {
      OnConnected(session);
    }

    internal void OnDisconnectingInternal(UdsSession session)
    {
      OnDisconnecting(session);
    }

    internal void OnDisconnectedInternal(UdsSession session)
    {
