  ...
  public class WebSocket : IWebSocket
  {
    ...
    public bool PerformClientUpgrade(HttpResponse response, Guid id)
    {
      ...
      for (var i = 0; i < response.Headers; i++)
      {
        ...
        else if (string.Compare(key, "Sec-WebSocket-Accept", StringComparison.OrdinalIgnoreCase) == 0)
        {
          ...
          var wskey = Convert.ToBase64String(WsNonce) + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
          string wshash;
    ...
    public bool PerformServerUpgrade(HttpRequest request, HttpResponse response)
    {
      ...
      for (var i = 0; i < request.Headers; i++)
      {
        ...
        {
          if (string.Compare(value, "Upgrade", StringComparison.OrdinalIgnoreCase) != 0 &&
              string.Compare(value.RemoveWhiteSpace(), "keep-alive,Upgrade", StringComparison.OrdinalIgnoreCase) != 0)
          {
            error = true;
    ...
    public void PrepareSendFrame(byte opcode, bool mask, ReadOnlySpan<byte> buffer, int status = 0)
    {
      ...
      var storeStatus = (opcode & WS_CLOSE) == WS_CLOSE && (buffer.Length > 0 || status != 0);
    ...
    public void PrepareReceiveFrame(byte[]? buffer, long offset, long size)
    {
      lock (WsReceiveLock)
      {
        ...
        while (buffer != null && size > 0)
        {
          ...
          var opcode = (byte)(WsReceiveFrameBuffer[0] & 0x0F);
          var fin = ((WsReceiveFrameBuffer[0] >> 7) & 0x01) != 0;
          var mask = ((WsReceiveFrameBuffer[1] >> 7) & 0x01) != 0;
          ...
          else if (payload == 127)
          {
            ...
            payload = (WsReceiveFrameBuffer[2] << 56) | (WsReceiveFrameBuffer[3] << 48) |
                      (WsReceiveFrameBuffer[4] << 40) | (WsReceiveFrameBuffer[5] << 32) |
                      (WsReceiveFrameBuffer[6] << 24) | (WsReceiveFrameBuffer[7] << 16) |
                      (WsReceiveFrameBuffer[8] << 8) | (WsReceiveFrameBuffer[9] << 0);
          ...
          var length = Math.Min(total - WsReceiveFrameBuffer.Size, size);
          ...
          if (WsReceiveFrameBuffer.Size == total)
          {
            ...
            if (mask)
              ...
              WsReceiveFinalBuffer.Append(WsReceiveFrameBuffer.AsReadOnlySpan()
                .Slice((int)WsHeaderSize, (int)WsPayloadSize));
