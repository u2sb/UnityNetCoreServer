...
namespace NetCoreServer
{
  ...
  public class UdpClient : IDisposable
  {
    ...
    private EndPoint _receiveEndpoint = null!;

    private EndPoint _sendEndpoint = null!;

    // Receive buffer
    private bool _receiving;
    private Buffer _receiveBuffer = new();

    private SocketAsyncEventArgs _receiveEventArg = new();

    // Send buffer
    private bool _sending;
    private Buffer _sendBuffer = new();
    private SocketAsyncEventArgs _sendEventArg = new();
    ...
    /// <returns>Size of sent data bytes</returns>
    public virtual long Send<T>(T data) where T : struct
    {
    ...
    /// <param name="data">Data to send</param>
    /// <returns>Size of sent data bytes</returns>
    public virtual long Send<T>(ReadOnlySpan<T> data) where T : struct
    {
    ...
    /// <param name="data">Data to send</param>
    /// <returns>Size of sent data bytes</returns>
    public virtual long Send<T>(EndPoint endpoint, T data) where T : struct
    {
      var span = MemoryMarshal.CreateReadOnlySpan(ref data, 1);
      return Send(endpoint, span);
    }
    ...
    /// <param name="data">Data to send</param>
    /// <returns>Size of sent data bytes</returns>
    public virtual long Send<T>(EndPoint endpoint, ReadOnlySpan<T> data) where T : struct
    {
      var span = MemoryMarshal.Cast<T, byte>(data);
      return Send(endpoint, span);
    }
    ...
    /// <param name="text">Text to send as a span of characters</param>
    /// <returns>Size of sent datagram</returns>
    public virtual long Send(EndPoint endpoint, ReadOnlySpan<char> text)
    {
    ...
    public virtual bool SendAsync<T>(T data) where T : struct
    {
      var span = MemoryMarshal.CreateReadOnlySpan(ref data, 1);
      return SendAsync(span);
    }
    ...
    /// <param name="data">Data to send</param>
    /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
    public virtual bool SendAsync<T>(ReadOnlySpan<T> data) where T : struct
    {
      var span = MemoryMarshal.Cast<T, byte>(data);
      return SendAsync(span);
    }
    ...
    /// <param name="data">Data to send</param>
    /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
    public virtual bool SendAsync<T>(EndPoint endpoint, T data) where T : struct
    {
      var span = MemoryMarshal.CreateReadOnlySpan(ref data, 1);
      return SendAsync(endpoint, span);
    }
    ...
    /// <param name="data">Data to send</param>
    /// <returns>'true' if the data was successfully sent, 'false' if the client is not connected</returns>
    public virtual bool SendAsync<T>(EndPoint endpoint, ReadOnlySpan<T> data) where T : struct
    {
      var span = MemoryMarshal.Cast<T, byte>(data);
      return SendAsync(endpoint, span);
    }
    ...
    /// <param name="text">Text to send as a span of characters</param>
    /// <returns>'true' if the text was successfully sent, 'false' if the text was not sent</returns>
    public virtual bool SendAsync(EndPoint endpoint, ReadOnlySpan<char> text)
    {
